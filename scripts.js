...
const draftListToPopulate = document.querySelector("#entryList");
const winnerListToPopulate = document.querySelector("#winnerList");
const selectedFilesContainer = document.querySelector("#selectedFiles");
const submitForm = document.querySelector("#submit");
const draftPlayers = document.querySelector("#draft");
const nameCollection = [];
let entryList = [];

function parsePersonName(name) {
  const utfFixedName = fixUtf8(name);
  const lowerCaseName = utfFixedName.toLowerCase();

  return lowerCaseName;
}

function getWinners() {
  const firstResult = Math.floor(Math.random() * entryList.length);
  const newEntryList = entryList.filter((_, index) => index !== firstResult);
  const secondResult = Math.floor(Math.random() * newEntryList.length);

  const firstWinner = document.createElement("li");
  const secondWinner = document.createElement("li");

  firstWinner.innerHTML = parsePersonName(entryList[firstResult]);
  secondWinner.innerHTML = parsePersonName(newEntryList[secondResult]);

  const emptyListSpan = document.querySelector(".winners-container .empty");
  emptyListSpan.innerHTML = "";

  winnerListToPopulate.append(firstWinner);
  winnerListToPopulate.append(secondWinner);
...
function readFileResult(fileResult) {
  if (!fileResult) {
    return;
  }

  const names = fileResult
    .split(",")
    .slice(4)
    .filter((string) => !/\d/.test(string));

  names.forEach((name) => nameCollection.push(name));
}

function readFile(file, isDone) {
  const reader = new FileReader();

  reader.onload = () => {
    readFileResult(reader.result);
...
function getAllDocs() {
  ...
  const allDocs = data.getAll("csv");
  const docsCount = allDocs.length;

  if (docsCount === 1) {
    submitForm.setAttribute("disabled", true);
    selectedFilesContainer.innerHTML =
      "Arquivos selecionados: 1 (Selecione mais de um arquivo)";
  } else {
    submitForm.removeAttribute("disabled");
    selectedFilesContainer.innerHTML = `Arquivos selecionados: ${docsCount}`;
  }

  return { allDocs, docsCount };
}

form.addEventListener("submit", (event) => {
  event.preventDefault();

  const { allDocs, docsCount } = getAllDocs();

  allDocs.forEach(async (file, index) => {
    const isDone = index + 1 === docsCount;
    readFile(file, isDone);
  });
  fileInput.value = "";
  submitForm.setAttribute("disabled", true);
});
